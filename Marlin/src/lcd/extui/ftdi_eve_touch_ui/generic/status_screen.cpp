...
void StatusScreen::draw_temperature(draw_mode_t what) {
  ...
  #define _ICON_POS(x,y,w,h) x, y, w/3.8, h               // I know this 3.8 should match the value below, but this aligns the icons better
  #define _TEXT_POS(x,y,w,h) x + w/3.3, y, w - w/3.3, h   // Changed to 3.3 to widen the button so text won't shrink when temps go to three digits
