...
void InterfaceSettingsScreen::onRedraw(draw_mode_t what) {
  ...
  if (what & BACKGROUND) {
    ...
    cmd.cmd(CLEAR_COLOR_RGB(bg_color))
       ...
       .tag(0)
       .font(font_medium)
       .text(BTN_POS(1,1), BTN_SIZE(4,1), F(GET_TEXTF(INTERFACE_SETTINGS)))
    #undef EDGE_R
    #define EDGE_R 30
       .font(font_small)
       .tag(0)
       .text(BTN_POS(1,2), BTN_SIZE(2,1), F(GET_TEXTF(LCD_BRIGHTNESS)), OPT_RIGHTX | OPT_CENTERY)
       .text(BTN_POS(1,3), BTN_SIZE(2,1), F(GET_TEXTF(SOUND_VOLUME)),   OPT_RIGHTX | OPT_CENTERY)
       .text(BTN_POS(1,4), BTN_SIZE(2,1), F(GET_TEXTF(SCREEN_LOCK)),    OPT_RIGHTX | OPT_CENTERY);
    cmd.text(BTN_POS(1,5), BTN_SIZE(2,1), F(GET_TEXTF(BOOT_SCREEN)),    OPT_RIGHTX | OPT_CENTERY);
    #undef EDGE_R
  }
  ...
  if (what & FOREGROUND) {
    ...
    #define EDGE_R 30
       ...
       .colors(ui_toggle)
       .tag(4).toggle2(BTN_POS(3,4), BTN_SIZE(w,1), F(GET_TEXTF(NO)), F(GET_TEXTF(YES)), LockScreen::is_enabled())
       .tag(5).toggle2(BTN_POS(3,5), BTN_SIZE(w,1), F(GET_TEXTF(NO)), F(GET_TEXTF(YES)), UIData::animations_enabled())
    #undef EDGE_R
    #define EDGE_R 0
    #ifdef TOUCH_UI_PORTRAIT
       .colors(normal_btn)
       .tag(6).button (BTN_POS(1,6), BTN_SIZE(4,1), F(GET_TEXTF(INTERFACE_SOUNDS)))
       .colors(action_btn)
       .tag(1).button (BTN_POS(1,7), BTN_SIZE(4,1), F(GET_TEXTF(BACK)));
    #else
       .tag(6).button (BTN_POS(1,6), BTN_SIZE(2,1), F(GET_TEXTF(INTERFACE_SOUNDS)))
       .colors(action_btn)
       .tag(1).button (BTN_POS(3,6), BTN_SIZE(2,1), F(GET_TEXTF(BACK)));
    #endif
  }
}
