...
class Polygon {
  ...
  public:
    ...
    void vertices(const uint16_t data[], size_t n, path_op p_op, vertex_op v_op) {
      uint16_t poly_start_x = 0xFFFF, poly_start_y = 0;
    ...
    void fill(const uint16_t data[], const size_t n) {
      ...
      vertices(data, n, &Polygon::draw_edge_strip_b, &Polygon::add_vertex_2f);
    ...
    void stroke(const uint16_t data[], size_t n) {
      vertices(data, n, &Polygon::draw_line_strip, &Polygon::add_vertex_2f);
    }
    ...
    void bounds(const uint16_t data[], size_t n, uint16_t &x, uint16_t &y, uint16_t &w, uint16_t &h) {
      ...
      x_max = 0;
      y_max = 0;
      vertices(data, n, &Polygon::noop, &Polygon::add_to_bounds);
      x = x_min/16;
      y = y_min/16;
      w = (x_max - x_min)/16;
      h = (y_max - y_min)/16;
