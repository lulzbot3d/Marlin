...
class FontScreen : public UIScreen, public UncachedScreen {
  public:
    static void onRedraw(draw_mode_t what) {
      ...
      cmd.font(31)
         .text(BTN_POS(1,1), BTN_SIZE(4,1), F("Héllô Wörld"))
         .font(30)
         .text(BTN_POS(1,2), BTN_SIZE(4,1), F("Ñôaæeïißú 80°F"))
         .tag(1)
         .button(BTN_POS(2,6), BTN_SIZE(2,1), F("Ökåý"));

      cmd.cmd(COLOR_RGB(0xFF9999))
         .rectangle(BTN_POS(1,3), BTN_SIZE(4,2))
         .cmd(COLOR_RGB(0x000000));
      draw_text_box(cmd, BTN_POS(1,3), BTN_SIZE(4,2), F("« La guerre, on le sait, fut pendant longtemps le plus sûr et le plus rapide véhicule de la civilisation. »\n\n- Jules Verne"), OPT_CENTERY);
    }
