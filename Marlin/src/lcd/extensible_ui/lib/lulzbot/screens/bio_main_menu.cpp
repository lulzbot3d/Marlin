...
void MainMenu::onRedraw(draw_mode_t what) {
  ...
  if (what & FOREGROUND) {
    ...
    cmd.cmd(COLOR_RGB(bg_text_enabled))
       ...
       .tag(5).button( BTN_POS(1,5), BTN_SIZE(2,1), GET_TEXTF(BED_TEMPERATURE))
       .tag(6).button( BTN_POS(1,6), BTN_SIZE(2,1), GET_TEXTF(INTERFACE_SETTINGS))
       .tag(7).button( BTN_POS(1,7), BTN_SIZE(2,1), GET_TEXTF(ADVANCED_SETTINGS))
       .tag(8).button( BTN_POS(1,8), BTN_SIZE(2,1), GET_TEXTF(ABOUT_PRINTER))
       .colors(action_btn)
...
bool MainMenu::onTouchEnd(uint8_t tag) {
  ...
  switch (tag) {
    ...
    case 3: SpinnerDialogBox::enqueueAndWait_P(F("G0 E0 F120"));                         break;
